...
export default function EmailAddMandatoryUsersToEventForm() {
    ...
    async function sendFormOnPress() {
        ...
        const response = await fetch(apiURL + "/events/"+event_id+"/mandatory/email", {
            method: "POST",
            headers: {
    ...
    return (
        <View className="w-11/12 mx-auto mt-5">
            <Text className="text-lg mt-2">Email címek:</Text>
            <TextInput
                editable
                multiline={true}
...
user4@example.com"
                ...
                onChange={(event) => { setEmails(event.nativeEvent.text.split("\n")) }}
            />
            {emailsError.length !== 0 && emailsError.map((error, i) => <Text key={i} className="text-red-500">{error}</Text>)}
